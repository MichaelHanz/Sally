...
const getCategoryForBOMItem = (itemName: string): string => {
  ...
  if (nameLower.includes('compute') || nameLower.includes('node') || nameLower.includes('server') || nameLower.includes('cpu') || nameLower.includes('gpu') || nameLower.includes('rtx') || nameLower.includes('xeon') || nameLower.includes('epyc') || nameLower.includes('laptop') || nameLower.includes('workstation')) {
    return 'Compute';
  }
  if (nameLower.includes('storage') || nameLower.includes('nvme') || nameLower.includes('ssd') || nameLower.includes('hdd') || nameLower.includes('drive') || nameLower.includes('sas') || nameLower.includes('flash') || nameLower.includes('array')) {
    return 'Storage';
  }
  if (nameLower.includes('switch') || nameLower.includes('network') || nameLower.includes('gbe') || nameLower.includes('infiniband') || nameLower.includes('cabling') || nameLower.includes('router') || nameLower.includes('ethernet')) {
    return 'Networking';
  }
  if (nameLower.includes('service') || nameLower.includes('architect') || nameLower.includes('engineer') || nameLower.includes('consult') || nameLower.includes('support') || nameLower.includes('delivery') || nameLower.includes('setup')) {
    return 'Services';
  }
  if (nameLower.includes('chair') || nameLower.includes('desk') || nameLower.includes('furniture') || nameLower.includes('standing')) {
    return 'Furniture';
  }
  if (nameLower.includes('monitor') || nameLower.includes('light') || nameLower.includes('dock') || nameLower.includes('deskpad') || nameLower.includes('peripheral') || nameLower.includes('screen')) {
    return 'Peripherals';
  }
  return 'Infrastructure';
};

const getPlatformForCategory = (itemName: string, category: string): { name: string; url: string } => {
  const query = encodeURIComponent(itemName);
  if (category === 'Furniture' || category === 'Peripherals') {
    return { name: 'Best Buy', url: `https://www.bestbuy.com/site/searchpage.jsp?st=${query}` };
  } else if (category === 'Compute' && itemName.toLowerCase().includes('laptop')) {
    return { name: 'Best Buy', url: `https://www.bestbuy.com/site/searchpage.jsp?st=${query}` };
  } else if (itemName.toLowerCase().includes('nvidia') || itemName.toLowerCase().includes('h100') || itemName.toLowerCase().includes('epyc')) {
    return { name: 'Industrial Tech B2B', url: `https://www.google.com/search?q=${query}+enterprise+supplier` };
  } else {
    return { name: 'Amazon Business', url: `https://www.amazon.com/s?k=${query}` };
  }
};

const getSourcingDiagnosticForProposal = (vertical: VerticalType, bomItems: any[]) => {
  const totalPowerDraw = bomItems.reduce((acc, bom) => {
    const item = CATALOG.find(c => c.name.toLowerCase() === bom.item.toLowerCase() || c.spec.toLowerCase() === bom.specification.toLowerCase());
  ...
  if (vertical === 'Smart Workspace') {
    return {
      platforms: [
        ...
        { name: 'Best Buy Specialty', status: 'Matched & Calibrated', matchCount: 5 },
        { name: 'Ergonomic Specialty Hub', status: 'Sourced & Audited', matchCount: 2 }
      ],
      rejections: [
        {
          item: 'Director Orthopedic Swivel Chair (Walmart)',
          reason: 'Lumbar support failed 24-point ergonomic safety threshold required for continuous 8hr screen operations.',
          criterion: 'Ergonomic Compliance'
        },
        {
          item: 'Dual Core Preconfigured Desktop (Retail Store)',
          reason: 'Passive cooling system and 450W power draw exceeded secure thermal layout profiles for compact workspaces.',
          criterion: 'Thermal / Form Fit'
        }
      ],
      validations: [
        { name: 'Ergonomic Desk Weight Load Tolerance', passed: true, detail: `Combined hardware weight (${totalWeight} kg) easily supported by standing desk motor capacity (fits safely within the max 120 kg tolerance).` },
        { name: 'Multi-device Thunderbolt Power Output', passed: true, detail: 'Integrated 180W Docking Station perfectly routes power up to 90 Watts over primary USB-C lines.' },
        { name: 'Acoustic Wool Sound Reflection Limit Check', passed: true, detail: '12 Acoustic self-adhesive felt panels successfully drop ambient room echo by an estimated 35%.' }
      ]
    };
  } else if (vertical === 'AI GPU Cluster') {
    return {
      platforms: [
        { name: 'NVIDIA Enterprise Wholesale', status: 'Sourced & Audited', matchCount: 3 },
        { name: 'Supercomputer Industrial Portal', status: 'Matched & Calibrated', matchCount: 4 },
        { name: 'Amazon Business Pro Division', status: 'Sourced & Audited', matchCount: 2 }
      ],
      rejections: [
        {
          item: 'Desktop Grade NVIDIA RTX 4090 GPU Node (Retail)',
          reason: 'Lacks native NVLink / ConnectX-7 NDR interconnect arrays, which throttles inter-GPU training bandwitdh by 82%.',
      ...
      validations: [
        { name: 'Quantum-2 400Gb/s InfiniBand Compatibility', passed: true, detail: 'High-speed OSFP non-blocking switches confirmed matching ConnectX-7 dual-port host adapters.' },
        { name: 'Double-Conversion Online UPS Safe Capacity', passed: true, detail: `Aggregated workload drawing ${totalPowerDraw.toLocaleString()} Watts is fully backed up by double-conversion SNMP battery modules.` },
        { name: 'Freight Logistics Air Cargo Compliance', passed: true, detail: `Shipment weight (${totalWeight} kg) cleared for heavy freight flight delivery with secure shock dampening packing standard.` }
      ]
    };
  } else {
    // Enterprise Server
    return {
      platforms: [
        { name: 'ServerMonkey Global Wholesale', status: 'Sourced & Audited', matchCount: 8 },
        { name: 'Newegg Business Infrastructure Portal', status: 'Matched & Calibrated', matchCount: 5 },
        { name: 'Amazon Business Pro Division', status: 'Sourced & Audited', matchCount: 3 }
      ],
      rejections: [
        {
          item: 'Mainstream SATA Enterprise HDD Tower (Best Buy)',
          reason: 'Rotational read speeds violate the absolute minimum target client database IOPS metrics of 250,500; replaced with Gen5 SSD arrays.',
          criterion: 'I/O Performance'
        },
        {
          item: 'Deep-Profile Multi-Node Rackmount Chassis (OEM)',
          reason: 'Physical chassis length exceeds maximum depth of standard server enclosures, causing severe hot-aisle air blockages.',
          criterion: 'Chassis Form Factor'
        }
      ],
      validations: [
        { name: 'Layer-3 ToR 100G Switch Redundancy', passed: true, detail: 'Double QSFP28 switches connected with active-active LACP links, guaranteeing zero downtime from single cable failures.' },
        { name: 'Smart Rack PDU Three-Phase Distribution', passed: true, detail: `Power load (${totalPowerDraw.toLocaleString()} Watts) balanced perfectly across smart switchable outlets.` },
        { name: 'Rack Weight Tolerances & Rail Mount Clearances', passed: true, detail: `Total server clusters (${totalWeight} kg) rest comfortably within the 42U heavy cabinet weight allowance (max 1000 kg load).` }
      ]
    };
  }
};
...
interface WorkflowsProps {
  ...
  predefinedBrief: { vertical: VerticalType; text: string; client: string; budget: number; location: string } | null;
...
export default function Workflows({ 
  ...
  const [viewState, setViewState] = useState<'prompt' | 'running' | 'resolved'>('prompt');
  
  // Prompt input text
  const [promptText, setPromptText] = useState('');
  
  // Simulating Console Steps Logs
  const [logs, setLogs] = useState<{ time: string; text: string; done: boolean; active: boolean }[]>([]);
  ...
  const extractParamsFromPrompt = (text: string) => {
    ...
    if (lowerText.includes('rm') || lowerText.includes('ringgit') || lowerText.includes('malaysian')) {
      extractedCurrency = 'MYR';
    }

    // Try finding price tokens
    const currencyMatch = lowerText.match(/(?:under|below|budget|limit|max|up to)?\s*(?:\$|rm|usd)?\s*([\d,]+)\s*(k)?/i);
    if (currencyMatch) {
    ...
    return { vertical, clientName, budgetLimit, deliveryLocation, currency: extractedCurrency };
  };
  ...
  const triggerConsultation = async () => {
    ...
    const initialLogs = [
      { time: getTimestamp(0), text: 'Initializing secure environment parameters... Done.', done: true, active: false },
      { time: getTimestamp(2), text: 'Authenticating user payload via primary token... Verified.', done: true, active: false },
      { time: getTimestamp(4), text: 'Searching catalog for matching infrastructure constraints... Found 14 items.', done: true, active: false },
      { time: getTimestamp(7), text: 'Validating power constraints against regional limits... Cleared.', done: true, active: false },
      { time: getTimestamp(10), text: 'Generating proposal structure based on optimal specs...', done: false, active: true },
    ];
    ...
    try {
      ...
      clearInterval(progressInterval);
      setErrorText(err.message || 'Consultation workflow compilation error. Please verify api routes.');
      setViewState('prompt');
    }
  };
  ...
  const handleModifyQty = (index: number, delta: number) => {
    ...
    let shipping_easyparcel = 15.00;
    if (totalWeight > 80) {
      shipping_easyparcel = 150.00 + (totalWeight - 80) * 1.25;
    } else {
      shipping_easyparcel = 15.00 + totalWeight * 0.75;
    }

    const isMalaysia = /malaysia|penang|kl|kuala|johor|selangor/i.test(currentProposal.delivery_location);
  ...
  const handleExportCSV = () => {
    ...
    // Construct rows
    const headers = ['Item', 'Specification', 'Qty', `Unit Price (${currency})`, `Total (${currency})`, 'Category'];
    const rows = currentProposal.bill_of_materials.map((bom) => {
      const itemPrice = currency === 'MYR' ? bom.unit_price * exchangeRate : bom.unit_price;
      const lineTotal = currency === 'MYR' ? bom.total * exchangeRate : bom.total;
    ...
    const csvContent = [
      ...
      [`"Project Title"`, `"${currentProposal.project_title.replace(/"/g, '""')}"`],
      [`"Client Name"`, `"${currentProposal.client_name.replace(/"/g, '""')}"`],
      [`"Delivery Location"`, `"${currentProposal.delivery_location.replace(/"/g, '""')}"`],
      [],
      headers,
      ...rows,
      [],
      ['Subtotal', '', '', '', (currency === 'MYR' ? currentProposal.financial_summary.subtotal * exchangeRate : currentProposal.financial_summary.subtotal).toFixed(2)],
      ['Shipping (EasyParcel)', '', '', '', (currency === 'MYR' ? currentProposal.financial_summary.shipping_easyparcel * exchangeRate : currentProposal.financial_summary.shipping_easyparcel).toFixed(2)],
      ['Tax (8.5%)', '', '', '', (currency === 'MYR' ? currentProposal.financial_summary.tax_amount * exchangeRate : currentProposal.financial_summary.tax_amount).toFixed(2)],
      ['Grand Total', '', '', '', (currency === 'MYR' ? currentProposal.financial_summary.grand_total * exchangeRate : currentProposal.financial_summary.grand_total).toFixed(2)]
    ].map(e => e.join(',')).join('\n');
    ...
    link.setAttribute('download', `proposal_${currentProposal.proposal_id || 'export'}.csv`);
        ...
        {viewState === 'prompt' && (
          ...
          >
            ...
            <p className="text-slate-600 text-sm md:text-base font-medium mt-3 text-center max-w-xl px-4 leading-relaxed">
              Autonomous Sales Engineer. Describe your client's needs, and I'll architect the solution.
            </p>
            ...
            <div className="flex flex-col items-center space-y-2 mt-8 select-none w-full max-w-xl">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2">
                <button 
                  type="button"
                  onClick={() => setPromptText('I need a minimalist home office for a 10x10ft room under RM5,000, delivering to Penang... please build the BOM with ergonomic workspace options')}
                ...
                <button 
                  type="button"
                  onClick={() => setPromptText('Architect a dual EPYC server cluster cluster with ultra low latency core switches under $150,000 bound for Acme Corp in Kuala Lumpur')}
                  className="px-3.5 py-1.5 bg-white border border-slate-200 hover:border-indigo-400 hover:bg-indigo-50 text-slate-700 hover:text-indigo-900 rounded-full transition text-[11px] font-semibold shadow-xs"
                >
                ...
                <button 
                  type="button"
                  onClick={() => setPromptText('We need a high-power Deep Learning GPU stack with NVIDIA hardware for our AI training lab at Maybank Bhd, budget limit under RM250,500')}
                  className="px-3.5 py-1.5 bg-white border border-slate-200 hover:border-indigo-400 hover:bg-indigo-50 text-slate-700 hover:text-indigo-900 rounded-full transition text-[11px] font-semibold shadow-xs"
                >
          ...
          >
            ...
            <div className="w-full max-w-xl bg-white border border-slate-200 rounded-2xl p-8 shadow-lg space-y-6 relative">
              ...
              <div className="text-center space-y-1">
                <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Agentic Engine Active</h2>
                <p className="text-slate-500 text-xs font-semibold uppercase tracking-wider">Executing workflow sequence autonomously.</p>
              </div>
              ...
              <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-5 space-y-3.5 font-mono text-xs text-slate-705">
                <AnimatePresence initial={false}>
                  {logs.map((log, idx) => (
                    ...
                    >
                      <span className="text-slate-400 w-16 shrink-0 select-none">[{log.time}]</span>
                      <div className="flex-1 ml-3 flex items-center justify-between">
                        <span className={`${log.active ? 'text-indigo-800 font-bold' : log.done ? 'text-slate-600' : 'text-slate-350'}`}>
                          {log.text}
                        </span>
                        {log.done && (
                          <span className="text-indigo-750 font-bold shrink-0 ml-2">Done.</span>
                        )}
                        {log.active && (
          ...
          >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-200">
              ...
              <div>
                ...
                <p className="text-slate-500 text-xs sm:text-sm font-medium mt-1">
                  Prepared for <strong className="text-slate-800 font-bold">{currentProposal.client_name}</strong> by Sally AI.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 items-start">
              ...
              <div className="lg:col-span-8 space-y-6">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 sm:p-6 shadow-sm space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
                    ...
                    <div className="flex items-center space-x-2 bg-slate-50 border border-slate-200 rounded-lg p-1 select-none">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider px-1">Display Currency:</span>
                      <button
                        type="button"
                    ...
                    <table className="w-full text-left font-sans text-xs">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {currentProposal.bill_of_materials.map((bom, idx) => {
                          ...
                          const platformInfo = getPlatformForCategory(bom.item, categoryName);
                          
                          return (
                            <tr key={idx} className="hover:bg-slate-50/20 transition group">
                              ...
                              <td className="p-3.5 text-right text-slate-600 font-medium">
                                {currencySymbol}{itemPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                              </td>
                ...
                {(() => {
                  const diagnostics = getSourcingDiagnosticForProposal(currentProposal.vertical, currentProposal.bill_of_materials);
                  return (
                    <div className="bg-white border border-slate-200 rounded-xl p-5 sm:p-6 shadow-sm space-y-5">
                      ...
                      <div className="space-y-2">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Sourcing Engines Consulted</span>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          {diagnostics.platforms.map((plat, i) => (
                            <div key={i} className="bg-slate-50 border border-slate-150 p-3 rounded-lg flex items-center justify-between">
                              <div className="min-w-0">
                                <p className="text-xs font-bold text-slate-700 truncate">{plat.name}</p>
                      ...
                      <div className="space-y-3 pt-1">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block font-mono">Strict Compatibility Evaluations</span>
                        <div className="space-y-2.5">
                          {diagnostics.validations.map((val, idx) => (
                            <div key={idx} className="bg-white border border-slate-100 hover:border-slate-200 rounded-lg p-3 sm:p-4 flex items-start gap-3 transition">
                              <div className="mt-0.5 bg-emerald-50 border border-emerald-100 text-emerald-700 p-1 rounded">
                                <Check className="h-3.5 w-3.5" />
                              </div>
                              <div className="space-y-1">
                                <p className="text-xs font-bold text-slate-800">{val.name}</p>
                                <p className="text-xs text-slate-500 leading-relaxed font-semibold">{val.detail}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="space-y-3 pt-1">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block font-mono">Automated Sourcing Reject log</span>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                          {diagnostics.rejections.map((rej, id) => (
                            <div key={id} className="bg-red-50/40 border border-red-100 hover:border-red-250 rounded-lg p-3.5 space-y-2 transition flex flex-col justify-between">
                              <div>
                                <div className="flex items-center justify-between gap-2">
                                  ...
                                  <span className="text-[9px] font-semibold text-slate-400">Rejected Match</span>
                                </div>
                                <h5 className="text-xs font-bold text-slate-850 mt-2 line-clamp-1">{rej.item}</h5>
                              ...
                              <div className="pt-2 border-t border-red-100/50 mt-2 flex items-center justify-between text-[10px] text-red-800 font-bold select-none">
                                <span className="flex items-center gap-1">
                                  <ShieldAlert className="h-3.5 w-3.5 text-red-650" /> Auto-Rejected By Sally
                                </span>
                                <span className="text-slate-400">0% Allocation</span>
                              </div>
                            </div>
                          ))}
                        </div>
                ...
                <div className="bg-[#FAF9FF] border border-indigo-100 rounded-xl p-5 sm:p-6 shadow-sm flex items-start space-x-4">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-slate-900 tracking-tight">Agent Reasoning</h4>
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-4">
                  ...
                  <div className="space-y-3.5 text-xs sm:text-sm text-slate-500 font-medium pt-1">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-slate-800">
                        {currencySymbol}{(currency === 'MYR' ? currentProposal.financial_summary.subtotal * exchangeRate : currentProposal.financial_summary.subtotal).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-slate-800">
                        {currencySymbol}{(currency === 'MYR' ? currentProposal.financial_summary.shipping_easyparcel * exchangeRate : currentProposal.financial_summary.shipping_easyparcel).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between border-b border-slate-100 pb-4">
                      <span>Tax (8.5%)</span>
                      <span className="font-bold text-slate-800">
                        {currencySymbol}{(currency === 'MYR' ? currentProposal.financial_summary.tax_amount * exchangeRate : currentProposal.financial_summary.tax_amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>

                    {/* Grand Total */}
                    <div className="flex justify-between items-baseline pt-2">
                      <span className="text-xs font-bold text-slate-800 uppercase tracking-widest select-none">Grand Total</span>
                      <span className="text-2xl font-black text-indigo-850 tracking-tight">
                        {currencySymbol}{(currency === 'MYR' ? currentProposal.financial_summary.grand_total * exchangeRate : currentProposal.financial_summary.grand_total).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
                  ...
                  {(() => {
                    ...
                    currentProposal.bill_of_materials.forEach((bom) => {
                      const cat = getCategoryForBOMItem(bom.item);
                      const value = currency === 'MYR' ? bom.total * exchangeRate : bom.total;
                      groupedBreakdown[cat] = (groupedBreakdown[cat] || 0) + value;
                    });

                    const pieData = Object.entries(groupedBreakdown).map(([name, value]) => ({
                      name,
                    ...
                    return (
                      <div className="border-t border-slate-100 pt-4 space-y-3">
                        ...
                        {pieData.length > 0 ? (
                          <>
                            ...
                            <div className="h-44 w-full flex items-center justify-center relative bg-slate-50/50 rounded-xl p-2 border border-slate-100 shadow-inner">
                              ...
                              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none select-none">
                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">BOM</span>
                                <span className="text-xs font-black text-indigo-900 mt-0.5">Share</span>
                              </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 text-[10px] sm:text-xs">
                              {pieData.map((item, idx) => {
                                ...
                                return (
                                  <div key={idx} className="flex items-center space-x-1.5 bg-slate-50 border border-slate-150 p-1.5 rounded-lg shadow-2xs">
                                    <span 
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <p className="text-[9px] text-slate-400 font-semibold uppercase leading-none mt-0.5">
                                        {percentage}% ({currencySymbol}{Math.round(item.value).toLocaleString()})
                                      </p>
                                    </div>
                                  </div>
                                );
                          ...
                          <p className="text-xs text-slate-400 text-center py-4">No materials added yet.</p>
                        )}
                      </div>
                    );
                  })()}
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-3">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block select-none">Consular Logistics Audit</span>
                  <div className="space-y-1.5 text-xs text-slate-600 font-medium">
                    ...
                    <div className="pt-2">
                      {currentProposal.financial_summary.subtotal <= currentProposal.budget_limit ? (
                        <span className="inline-flex items-center text-[10px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded">
                          <Check className="h-3 w-3 mr-1" /> Budget limit respected
                        </span>
                      ) : (
                        <span className="inline-flex items-center text-[10px] font-bold text-amber-600 bg-amber-50 border border-amber-100 px-2 py-0.5 rounded">
                          <AlertCircle className="h-3 w-3 mr-1" /> Warning: Overdraft limit
                        </span>
                      )}
                    </div>
                  </div>
